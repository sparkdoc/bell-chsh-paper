...
\begin{abstract}
We compute the exact minimum measurement dependence that a local
hidden-variable model needs in order to reproduce the maximal quantum
violation of the CHSH inequality, measured as the worst-case total-variation
distance $F=\max_{ab}\,\TV(p(\cdot\mid a,b)\,\Vert\,\rho)$ between the
settings-conditioned hidden-variable distribution and a fixed baseline
$\rho$. Every local model --- on any hidden-variable space, with any source
distribution and deterministic or stochastic responses --- obeys
$S\le 2+8F$, so reaching the Tsirelson value $2\sqrt2$ costs at least
$F^*=(\sqrt2-1)/4\approx0.104$. This floor is attained by an explicit
four-state model, verified as an exact identity in $\mathbb{Q}(\sqrt2)$,
and adding hidden states beyond four never lowers the cost: for $N$-state
uniform-source models $F^*(N)=(\sqrt2-1)/\min(N,4)$. The same minimum is
achievable without observable signalling for every $N\ge3$, with explicit
no-signalling witnesses, while no two-state model can violate Bell under
no-signalling at any fine-tuning level. We further give the exact
piecewise-linear curves $S_{\max}(N,F)$ for $N\le14$ by exhaustive exact
arithmetic, the cost $\lceil N/4\rceil/N$ of reaching the algebraic maximum
$S=4$ with or without no-signalling (where the unique no-signalling box is
the PR box), the one-sided value $(\sqrt2-1)/2$, and conversion bounds to
Hall's measure and to mutual information. The qualitative lesson that
modest measurement dependence suffices is known; the contribution is an
exact, certified, model-independent price in one natural metric, together
with the observation that it can be paid without leaving a trace in the
...
nonlocality for suitable correlations \cite{putz2014,putzgisin2016}.
...
bound $S\le 2+8F$; (ii) the no-signalling sub-case pinned to the same closed
...
\emph{Metric dependence of the answer.} How much measurement dependence is
``enough'' is known to depend strongly on both the metric and the target
correlations. In the polytope parametrisation of P\"utz \emph{et al.}, any
nonzero residual measurement independence can be detected by suitable
quantum correlations \cite{putz2014,putzgisin2016}; in Hall's $M$
\cite{hall2010}, in mutual information \cite{barrettgisin2011,
hallbranciard2020}, and in our per-setting TV metric, a modest and
quantitatively fixed amount reproduces the CHSH Tsirelson value. Our
result should be read as one exactly solved point in that metric-dependent
landscape, not as a metric-independent price of the loophole.
...
curves and cap $\lceil N/4\rceil/N$, or (iii) an NS-silence result of this
kind. This is stated as a search result, not ``proven novel'': coverage was
...
Bell/CHSH in a per-setting total-variation metric. The price has a closed
form: $F^*=(\sqrt2-1)/\min(N,4)$, saturating at four hidden states; the
global bound $S\le2+8F$ is model-independent and tight; the cap to the
algebraic maximum is $\lceil N/4\rceil/N$; the no-signalling constraint is
exactly silent about the minimum for all $N\ge3$ --- at both the Tsirelson
value and the algebraic maximum, where the unique NS box is the PR box ---
with explicit algebraic witnesses, while at $N=2$ it forbids any violation;
and the one-sided causal structure pays exactly twice the full-structure
cost, flat in $N$. The superdeterminism loophole, in this metric, costs a
concrete $\approx10.4\%$ of TV per settings block --- and that cost can be
hidden from the observed marginals. The systematic comparison with
